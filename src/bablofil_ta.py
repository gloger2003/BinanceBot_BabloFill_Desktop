...
def STOCH(high, low, closes, fastk_period, slowk_period, slowd_period):
    ...
    for i, _ in enumerate(closes):
        if (i + 1) < fastk_period:
            ...
            fastk.append(((closes[i] - curr_low) /
                          (curr_high - curr_low)) * 100)
...
def BBANDS(data, ma=SMA, ma_period=20, dev_val=2):
    ...
    # calculating stddev. We won't count NaN values.
    # Also NaNs are reasons not to use statistics.stddev, numpy, etc.
    stddevs = []
    real_data_cnt = 0
    ...
    for i in range(len(data)):
        ...
        if i-real_data_cnt >= ma_period:
            ...
            s = sum(map(lambda x: math.pow(x - avg, 2),
                        data[i-ma_period+1:i+1]))
