...
# Если передать параметр increase=True,
# то округление произойдет к следующему шагу
...
class TimeSyncerThread(QThread):
    def __init__(self, parent: QMainWindow, bot: Binance,
            ...
            try:
                ...
                if local_time + shift_seconds != server_time:
                    ...
                    log.debug("""
                        ...
                        Бот будет работать, как будто сейчас: {fake_time_d} '
                        '{fake_time_u}
                    """.format(
                        ...
                        warn="ТЕКУЩЕЕ ВРЕМЯ ВЫШЕ" if local_time > server_time
                        else '',
                        fake_time_d=datetime.fromtimestamp(local_time +
                                                           shift_seconds),
...
def calc_buy_avg_rate(order_trades, log):
    ...
    for trade in order_trades:
        ...
        log.debug(
            'По ордеру была сделка {id} на покупку {am:0.8f} '
            'по курсу {r:0.8f}, комиссия {fee:0.8f} {f_a}'.format(
                id=trade.trade_id,
...
def calc_sell_avg_rate(order_trades, log):
    ...
    for trade in order_trades:
        ...
        log.debug(
            'По ордеру была сделка {id} на продажу {am:0.8f} '
            'по курсу {r:0.8f}, комиссия {fee:0.8f} {f_a}'.format(
                id=trade.trade_id,
                am=trade.trade_amount,
                r=trade.trade_rate,
