...
# from config import (bot, pairs, log, TIMEFRAME, KLINES_LIMITS,
#                     POINTS_TO_ENTER, USE_OPEN_CANDLES)
from misc import adjust_to_step, calc_buy_avg_rate, calc_sell_avg_rate, get_order_trades
...
class BinanceBotThread(QThread):
    def __init__(self, parent: QMainWindow,
                 logger: Logger, config: Config) -> None:
        super().__init__(parent)

        self.__logger = logger
        self.__config = config

        self.__binance_bot = Binance(
            API_KEY=self.__config.api_key,
            API_SECRET=self.__config.api_secret
        )

    def run(self):
        # Переопределение переменных для бота
        bot = self.__binance_bot
        ...
        pairs = [{
            ...
            # Сколько тратить base каждый раз при покупке quote
            'spend_sum': k.spend_sum,

            # Какой навар нужен с каждой сделки? (1=1%)
            'profit_markup': k.profit_markup,

            # Нужно ли продавать с убытком при падении цены
            'use_stop_loss': k.use_stop_loss,

            # 1% - На сколько должна упасть цена, что бы продавать с убытком
            'stop_loss': k.stop_loss,
            'active': k.active,
        } for k in self.__config.pairs.values()]
        
        # Убрать
        from pprint import pformat
        log.info(pformat(pairs))


        TIMEFRAME = self.__config.timeframe
        KLINES_LIMITS = self.__config.klines_limits
        POINTS_TO_ENTER = self.__config.points_to_enter
        USE_OPEN_CANDLES = self.__config.use_open_candles
        ...
        while True:
            try:
                ...
                if orders_info:
                    log.debug("Получены неисполненные ордера из БД: {orders}".format(
                    ...
                    for order in orders_info:
                        ...
                        if orders_info[order]['order_type'] == 'buy':
                            if not orders_info[order]['buy_verified']:
                                ...
                                if avg_rate > 0:
                                    update_buy_rate(
                                        cursor, conn, orders_info[order]['order_id'], avg_rate)
                                else:
                                    log.debug(
                            ...
                            if order_status == 'FILLED':
                                ...
                                for elem in limits['symbols']:
                                    ...
                                    raise Exception(
                                        "Не удалось найти настройки выбранной пары " + pair_name)
                                ...
                                price_change = (
                                    curr_rate/orders_info[order]['buy_price']-1)*100
                                log.debug("Цена изменилась на {r:0.8f}%, процент для продажи {sp:0.8f}".format(
                                    r=price_change, sp=all_pairs[stock_order_data['symbol']]['profit_markup']))
                                if price_change >= all_pairs[stock_order_data['symbol']]['profit_markup']:
                                    ...
                                    if 'orderId' in new_order:
                                        log.info("Создан ордер на продажу по рынку {new_order}".format(
                                            new_order=new_order))
                                        store_sell_order(
                                            cursor, conn, order, new_order['orderId'], got_qty, 0)
                                        ...
                                        if avg_rate > 0:
                                            update_sell_rate(
                                                cursor, conn, new_order['orderId'], avg_rate)
                                        else:
                                            log.debug(
                                                "Не удается вычислить цену покупки, пропуск")
                                            continue
                                    ...
                                    # Если были ошибки при создании, выводим сообщение
                                        ...
                                        log.warning("Не удалось создать ордер на продажу {new_order}".format(
                                            new_order=new_order))
                                else:
                            ...
                            if order_status == 'FILLED' and orders_info[order]['order_type'] == 'buy':
                                ...
                                if (1 - curr_rate/orders_info[order]['buy_price'])*100 >= all_pairs[orders_info[order]['order_pair']]['stop_loss']:
                                    ...
                                    for elem in limits['symbols']:
                                        ...
                                        raise Exception(
                                            "Не удалось найти настройки выбранной пары " + orders_info[order]['order_pair'])
                ...
                if all_pairs:
                    log.debug('Найдены пары, по которым нет неисполненных ордеров: {pairs}'.format(
                    ...
                    for pair_name, pair_obj in all_pairs.items():
                        try:
                            ...
                            if enter_points < POINTS_TO_ENTER:
                                log.debug("Минимальный проходной балл {b}. Пропуск пары".format(
                                    b=POINTS_TO_ENTER))
                                continue
                            ...
                            if balances[pair_obj['base']] >= pair_obj['spend_sum']:
                                ...
                                my_amount = adjust_to_step(
                                    pair_obj['spend_sum']/top_price, CURR_LIMITS['filters'][2]['stepSize'])
                                ...
                                if trade_am < float(CURR_LIMITS['filters'][3]['minNotional']):
                                    raise Exception("""
                                        ...
                                        trade_am=trade_am, min_am=float(
                                            CURR_LIMITS['filters'][3]['minNotional']),
                                        incr=float(
                                            CURR_LIMITS['filters'][3]['minNotional'])/trade_am
                                    ))
                                log.debug(
                                    'Рассчитан ордер на покупку по рынку: кол-во {amount:0.8f}, примерный курс: {rate:0.8f}'.format(
                                        amount=my_amount, rate=top_price)
                                )
                                ...
                                if 'orderId' in new_order:
                                    log.info("Создан ордер на покупку {new_order}".format(
                                        new_order=new_order))

                                    add_db_new_order(
                                        cursor, conn, pair_name, new_order['orderId'], my_amount, top_price)
                                    ...
                                    log.warning("Не удалось создать ордер на покупку! {new_order}".format(
    ...
    def get_binance_bot(self):
        return self.__binance_bot
